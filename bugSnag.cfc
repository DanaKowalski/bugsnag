	...
	public struct function notify_bug(
		...
		if(!return_data.errors.len()){
			try {
				...
				http_call.addParam(type="body", value=serializeJSON(payload));

				http_call.addParam(type="header", name="Content-Type", value="application/json");
				http_call.addParam(type="header", name="Bugsnag-Payload-Version", value=variables.api_payload_version);
				http_call.addParam(type="header", name="Bugsnag-Api-Key", value=variables.api_key);
				http_call.addParam(type="header", name="Bugsnag-Sent-At", value=send_date_time);
				
				return_data.response = http_call.send().getPrefix();

				if(return_data.response.keyExists("statusCode") && return_data.response.statusCode.find("200")){
